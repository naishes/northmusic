module.exports = {
	listpatrocinio(callback) {
		var sql = 'SELECT * from patrocinio';
    ...
    readpatrocinio(nome_patrocinio, callback) {
		var sql = "SELECT * from patrocinio where nome_patrocinio=?";	
    ...
    updatepatrocinio(nome_patrocinio, data, callback) {
		var sql = "UPDATE patrocinio SET valor_patrocinio=? WHERE nome_patrocinio=?"; 
    ...
    removepatrocinio(nome_patrocinio, callback) {
		var sql = "DELETE from patrocinio WHERE nome_patrocinio=?"; 
		global.connection.query(sql, [nome_patrocinio], function(error, rows, fields){
			if (error) throw error;
			callback(rows);
		});
	...
	createpatrocinio(data, callback) {
		var sql = "INSERT INTO patrocinio (nome_patrocinio, valor_patrocinio) VALUES (?,?)"; 
		global.connection.query(

module.exports = {
		...
		createsessao(data, callback) {
			var sql = "INSERT INTO sessao (titulo_sessao, dia_sessao, hora_sessao, id_speaker, descricao_sessao, hora_fim) VALUES (?,?,?,?,?,?)"; 
			...
			horaExists(data,callback){
				var sql = "SELECT * from sessao WHERE dia_sessao=? and (? >= hora_sessao and ? <= hora_fim or ? >= hora_sessao and ? <= hora_fim or ? < hora_sessao and ? > hora_fim)";
				global.connection.query(
